...
def make_io_dataset(lines, input_names, output_names):
    """
    Return dataset as list of traces, each a sequence of (input, output) pairs.
    """
    dataset = []
    for line in lines:
        trace = parse_binary_trace(line, input_names, output_names)
        dataset.append(trace)  # trace is already [(input, output), ...]
    return dataset
...
def main():
    ...
    dataset = make_io_dataset(lines, input_names, output_names)
    print(f"Loaded {len(lines)} traces, created dataset with {len(dataset)} sequences.")
