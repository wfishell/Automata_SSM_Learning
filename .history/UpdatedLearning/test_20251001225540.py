...
def parse_formula_side(side):
    """Return dict {ap: 0/1} from a conjunction like 'a & !b'."""
    literals = {}
    if not side:
        return literals
...
def simplify_disjunction(expr):
    """Pick one disjunct (first) from '(... ) | (... )'."""
    if "|" in expr:
        return expr.split("|", 1)[0].strip("() ")
    return expr.strip("() ")

def step_to_spot(step, ap_order):
    """Convert 'inp/out' to full Spot semantics valuation."""
    if "/" in step:
    ...
    # choose one disjunct if | present
    ...
    # fill all APs in order
    return "&".join(ap if literals.get(ap, 0) else f"!{ap}" for ap in ap_order)
...
def main():
    ...
    for i in range(args.num):
        ...
        print(trace_to_spot(raw_trace, ap_order))
