    ...
    with open(filename, "w") as f:
        ...
        for s in states:
            ...
            for inp, next_state in s.transitions.items():
                ...
                lits = []
                for i, val in enumerate(inp + out):
                    if val == 1:
                        lits.append(str(i))
                    else:
                        lits.append(f"!{i}")
...
if __name__ == "__main__":
    ...
    INPUTS = [ap.strip() for ap in args.ins.split(",")]
    OUTPUTS = [ap.strip() for ap in args.outs.split(",")]

    dataset = process_file(args.trace_file, INPUTS, OUTPUTS)
    ...
    out_file = str(Path(args.trace_file).with_suffix(".hoa"))
