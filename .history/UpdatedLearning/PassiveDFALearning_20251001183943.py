"""
Trace Converter + RPNI DFA Learner (Positive/Negative Traces)
- Positive traces appear above "------" divider
- Negative traces appear below divider
- Prefix-closed dataset: prefixes of positives labeled True, prefixes of negatives labeled False
...
def make_prefix_closed(trace: List[Tuple[int, ...]], label: bool):
    """Return all prefixes of a trace, each labeled with True/False."""
    prefixes = []
    for i in range(1, len(trace) + 1):
        prefixes.append((tuple(trace[:i]), label))
    return prefixes
...
def process_file(trace_file: str, aps: List[str]):
    ...
    for i, line in enumerate(pos_lines):
        ...
        prefix_traces = make_prefix_closed(trace, True)
        dataset.extend(prefix_traces)
        print(f"[Pos Trace {i+1}] expanded into {len(prefix_traces)} True prefixes")

    # Negative traces → labeled False
    for i, line in enumerate(neg_lines):
        trace = parse_trace(line, aps)
        prefix_traces = make_prefix_closed(trace, False)
        dataset.extend(prefix_traces)
        print(f"[Neg Trace {i+1}] expanded into {len(prefix_traces)} False prefixes")
...
if __name__ == "__main__":
    ...
    # Run RPNI with positive and negative prefixes
