...
def parse_binary_trace(line: str, input_names, output_names):
    """Convert one raw binary trace line into a pair (input_seq, output_seq)."""
...
def convert_i_o_traces_for_RPNI(io_traces):
    """
    Input: list of traces, each as (input_symbol_list, output_symbol_list).
...
def main():
    ...
    print(f"Created dataset with {len(dataset)} unique prefix-output pairs.")
