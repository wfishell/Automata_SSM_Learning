...
def parse_binary_trace(line: str, input_names, output_names):
    """Convert one raw binary trace line into (input_sequence, output_sequence)."""
    ...
    for idx, step in enumerate(steps):
        ...
        in_seq.append(in_tok)
        out_seq.append(out_tok)
...
def main():
    ...
    # Step 2: convert using AALPy helper (prefix-closed dataset)
    dataset = convert_i_o_traces_for_RPNI(raw_dataset)
    print(f"Converted dataset to {len(dataset)} prefix-output pairs.")

    # Step 3: learn Mealy machine
    mealy = run_RPNI(data=dataset, automaton_type="mealy", algorithm="classic")
    if mealy is None:
        print("RPNI could not learn a deterministic Mealy machine from the provided data.", file=sys.stderr)
        sys.exit(1)
