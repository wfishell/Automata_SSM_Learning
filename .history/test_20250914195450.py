...
def parse_binary_trace(line: str, input_names, output_names):
    """Convert one raw binary trace line into aligned input/output token lists."""
...
def main():
    ...
    # Step 1: parse traces into sequences of (input, output) pairs
    dataset = []
    for line in lines:
        in_seq, out_seq = parse_binary_trace(line, input_names, output_names)
        trace = list(zip(in_seq, out_seq))
        dataset.append(trace)

    print(f"Loaded {len(lines)} traces.")

    # Step 2: run RPNI with Mealy dataset
    mealy = run_RPNI(data=dataset, automaton_type="mealy", algorithm="classic")
    if mealy is None:
        print("RPNI failed: dataset may be non-deterministic.", file=sys.stderr)
        sys.exit(1)
