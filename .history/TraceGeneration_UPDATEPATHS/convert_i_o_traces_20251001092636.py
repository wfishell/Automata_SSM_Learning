...
def parse_step(
    ...
    # inputs stay as a dict
    in_dict = {k: valuation.get(k, 0) for k in inputs}

    # outputs flattened into a single string symbol
...
def process_file(trace_file: str, inputs: List[str], outputs: List[str]):
    ...
    for i, line in enumerate(lines):
        ...
        # 🔹 Call AALpy conversion
        converted = convert_i_o_traces_for_RPNI(trace)

        # RPNI expects (word, label), where word is a list of symbols
        dataset.append((converted, True))  # assume all traces are positive
...
if __name__ == "__main__":
    # Define inputs/outputs (adjust to your system)
